...
class DataThread(QtCore.QThread):
    ...
    def run(self):
        ...
        micron_feed1=aio.feeds('sensor-1')
        micron_feed2=aio.feeds('sensor-2')
        micron_feed3=aio.feeds('sensor-3')
        ...
        while True:
            ...
            self.temp1=aio.receive(micron_feed1.key).value
            self.temp2=aio.receive(micron_feed2.key).value
            self.temp3=aio.receive(micron_feed3.key).value

            self.value1=self.temp1.strip('][').split(',')
            self.value2=self.temp2.strip('][').split(',')
            self.value3=self.temp3.strip('][').split(',')
            ...
            if idx <= window_size:
                ...
                self.yval1.append(self.value1[0])
                self.yval2.append(self.value2[0])
                self.yval3.append(self.value3[0])

                self.stat1.append(self.value1[1])
                self.stat2.append(self.value2[1])
                self.stat3.append(self.value3[1])
